...
cons1 = cons1 %>% 
  mutate_at(vars(cs02a_12:cs02a_12),  ~ . *4.3) 
...
df_cons_3month %>% names()

## B.3.1 Select 3 month consumption data
...
## B.4.1 Select yearly consumption data
cons4 = df_cons_3month %>% 
  select(
    "Household_ID", "cs27a_2", "cs27b_2", "cs27c_2", "cs27d_2", "cs27e_2", 
    "cs27f_2","cs31_2" )

## B.3.2 Divide data by 12
cons4 = cons4 %>% 
  mutate_at(vars(cs27a_2:cs27f_2),  ~ . /12) 


# B.5 Merge consumption variables into one dataframe
## B.5.1 Merge consum by house id

df_cons_merge = merge(cons1, cons3, by='Household_ID')

df_cons_merge = merge(df_cons_merge, cons2, by='Household_ID')

df_cons_merge = merge(df_cons_merge, cons4, by='Household_ID')
...
df_cons_merge = df_cons_merge %>% 
  mutate(
    percap_cons = total_cons/members #Change these names for your assignment to reflect the column names
  )
...
## 2.1 Head countdf_cons_merge %>% 
df_cons_merge = df_cons_merge %>% mutate(pov_dummy = as.numeric(percap_cons<povertyline))
...
df_cons_merge %>% 
  mutate(pov_dummy = as.numeric(percap_cons<povertyline),
         pov_members = pov_dummy*members
  ) %>% select(pov_dummy, pov_members, percap_cons, members) %>% 
  head(10)
...
df_cons_merge %>% 
  ...
  summarise(sum(pov_members)/sum(members))
...
df_cons_merge %>% 
  filter(pov_dummy==1) %>% 
  mutate(poverty_gap = povertyline-percap_cons) %>% 
...
df_cons_merge %>% 
  filter(pov_dummy==1) %>% 
  mutate(poverty_gap = povertyline-percap_cons,
         poverty_gap_members = mean(poverty_gap*members)
         ) %>% 
  summarise(mean( poverty_gap_members))
...
df_cons_merge %>% 
  filter(pov_dummy==1) %>% 
  mutate(poverty_gap_sq = (povertyline-percap_cons)^2) %>% 
  summarise(mean(poverty_gap_sq))
...
df_cons_merge %>% 
  filter(pov_dummy==1) %>% 
  mutate(poverty_gap = povertyline-percap_cons,
         poverty_gap_members = mean(poverty_gap*members)**2
  ) %>% 
  summarise(mean( poverty_gap_members))
...
#### 3.1.3.1 Create urban dummy
df_cons_merge = df_cons_merge %>% 
  mutate(
    urban_dummy = as.numeric(estrato<=2),
   poverty_gap = povertyline-percap_cons,
   pov_members = pov_dummy*members
  )
  

#### 3.1.3.2 Calculate poverty rate by residence
df_cons_merge %>% 
  group_by(urban_dummy) %>% 
  summarise(
    poverty_rate = mean(pov_dummy)
  )
...
df_cons_merge %>% 
  group_by(urban_dummy) %>% 
  summarise(
    poverty_rate = sum(pov_members)/sum(members)
  )



### 3.2  What can you say about the severity of poverty in rural versus urban areas?

df_cons_merge %>% 
  filter(pov_dummy==1) %>% 
  group_by(urban_dummy) %>% 
  summarise(
    poverty_gap = mean(poverty_gap),
    poverty_gap_sq = mean(poverty_gap)^2
  )
